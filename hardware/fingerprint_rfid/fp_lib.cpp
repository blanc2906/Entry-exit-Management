...
uint8_t Adafruit_Fingerprint::getTemplateIndexTable(uint8_t page) {
  GET_CMD_PACKET(FINGERPRINT_TEMPLATE_INDEX_TABLE, page);
  
  memset(indexTable, 0xff, 32);
  for (int i = 0; i < 32; i++)
    indexTable[i] = packet.data[i + 1];
